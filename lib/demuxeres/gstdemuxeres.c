...
#define GST_CAT_DEMUXER_ES video_demuxer_es_debug
...
appsink_new_sample_cb (GstAppSink * appsink, gpointer  user_data)
{
  ...
  GstDemuxerESPrivate *priv = stream->demuxer->priv;
...
gst_demuxer_es_new (const gchar * uri)
{
  ...
  GST_DEBUG_CATEGORY_INIT (demuxer_es_debug, "demuxer_es",
      GST_DEBUG_BOLD, "demuxer es");
  ...
  //FIXME: Waiting more is generating a leak, see https://gitlab.freedesktop.org/gstreamer/gstreamer/-/issues/1346#note_1670140
  if (demuxer && !wait_for_demuxer_ready (demuxer, G_TIME_SPAN_SECOND)) {
    GST_ERROR ("The demuxer never got ready");
    gst_demuxer_es_teardown (demuxer);
    demuxer = NULL;
  }
  return demuxer;
