...
struct VkParserH264PictureData {
    // SPS
...
struct VkParserHevcPictureData {
    // SPS
...
struct VkParserAv1GlobalMotionParameters {
    uint32_t wmtype;
    int32_t wmmat[6];
    int8_t invalid;
    int8_t reserved[3];
...
struct VkParserSequenceInfo {
    VkVideoCodecOperationFlagBitsKHR eCodec; // Compression Standard
    bool isSVC;              // h.264 SVC
    uint32_t frameRate;      // Frame Rate stored in the bitstream
    ...
    int32_t lDARWidth, lDARHeight; // Display Aspect Ratio = lDARWidth : lDARHeight
    ...
    int32_t nMinNumDecodeSurfaces; // Minimum number of decode surfaces for
                                   // correct decoding
    uint8_t SequenceHeaderData[1024]; // Raw sequence header data
                                      // (codec-specific)
...
struct VkParserOperatingPointInfo {
...
public:
    virtual int32_t BeginSequence(const VkParserSequenceInfo*) = 0; // Returns max number of reference frames
                                                                    // (always at least 2 for MPEG-2)
    virtual bool AllocPictureBuffer(VkPicIf**) = 0; // Returns a new VkPicIf interface
    virtual bool DecodePicture(VkParserPictureData*) = 0; // Called when a picture is
                                                          // ready to be decoded
    virtual bool UpdatePictureParameters(VkPictureParameters*, VkSharedBaseObj<VkParserVideoRefCountBase>&, uint64_t) = 0; // Called when a picture is
                                                                                                                           // ready to be decoded
    virtual bool DisplayPicture(VkPicIf*, int64_t) = 0; // Called when a picture is ready to be displayed
    virtual void UnhandledNALU(const uint8_t*, int32_t) = 0; // Called for custom NAL parsing (not required)
    virtual uint32_t GetDecodeCaps() { return 0; } // NVD_CAPS_XXX
    virtual int32_t GetOperatingPoint(VkParserOperatingPointInfo*) { return 0; } // called from sequence header of av1 scalable video streams

protected:
    virtual ~VkParserVideoDecodeClient() { }
};

struct VkParserInitDecodeParameters {
    uint32_t interfaceVersion;
    VkParserVideoDecodeClient* pClient; // should always be present if using parsing
                                        // functionality
    uint64_t lReferenceClockRate;       // ticks per second of PTS clock
                                        // (0=default=10000000=10Mhz)
    int32_t  lErrorThreshold;           // threshold for deciding to bypass of picture
...
struct VkParserBitstreamPacket {
...
struct VkParserDisplayMasteringInfo {
    // H.265 Annex D.2.27
...
// High-level interface to video decoder (Note that parsing and
// decoding functionality are decoupled from each other)
class VulkanVideoDecodeParser
    : public virtual VkParserVideoRefCountBase {
public:
    virtual VkResult Initialize(VkParserInitDecodeParameters*) = 0;
    virtual bool Deinitialize() = 0;
    virtual bool DecodePicture(VkParserPictureData*) = 0;
    virtual bool ParseByteStream(const VkParserBitstreamPacket*, int32_t* = NULL) = 0;
    virtual bool DecodeSliceInfo(VkParserSliceInfo*, const VkParserPictureData*, int32_t) = 0;
    virtual bool GetDisplayMasteringInfo(VkParserDisplayMasteringInfo*) = 0;
};
